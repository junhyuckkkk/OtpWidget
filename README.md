...
- 코드를 클릭하면 클립보드에 복사 (`복사됨!` 표시)
- **연속 붙여넣기**: 계정을 여러 개 차례로 클릭하면 붙여넣을 때마다 다음 계정 코드가 붙여집니다 (예: ERP 클릭 → GW 클릭 → 첫 Ctrl+V는 ERP 코드, 둘째 Ctrl+V는 GW 코드). 필요한 계정을 먼저 다 클릭한 뒤 붙여넣기를 시작하세요. Ctrl+V와 Shift+Insert만 인식하고 마우스 우클릭 붙여넣기는 다음 코드로 넘어가지 않습니다. 대기열은 2분 뒤 자동으로 비워지고, 그 사이 다른 것을 복사하면 위젯은 클립보드를 건드리지 않습니다.
- 남은 시간 바 (5초 이하면 빨간색)
